...
class Section_Momentum_BackTest(BackTest):
    ...
    def init(self, context):
        ...
        self.vol = pd.read_excel("data/future_std.xlsx")
        #print(self.data)
    ...
    def handle_bar(self, m_data, context):
        ...
        if not context.fired:
            ...
            usage=1
            try:
                usage=min(self.vol.loc[self.current]/context.S,1)
            except:
                usage=1
...
if(__name__=="__main__"):
    ...
    for n in [0.05,0.06,0.07,0.08,0.09,0.10,0.11,0.12]:
        for h in range(14,21):
            ...
            engine.context.H=2
            engine.context.S=n
            engine.context.name = f"newsecbreakallvol_R{h}_S{n:.2f}_test"
            p.apply_async(engine.loop_process,args=("20120101","20240501","back/section/newsecbreakallvol/"))
