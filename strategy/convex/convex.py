    ...
    def handle_bar(self, m_data, context):
        ...
        if not context.fired:
            ...
            for future_type in context.typelist:
                ...
                try:
                    if len(m_data[future_type])<=2+2*context.N+context.M:
                        continue
                    convex = m_data[future_type]["profit"].iloc[-2-context.M]+m_data[future_type]["profit"].iloc[-2-context.M-2*context.N]-2*m_data[future_type]["profit"].iloc[-2-context.M-context.N]
...
if(__name__=="__main__"):
    for n in [10,20,60,120,180,240]:
        for m in [0,1,2,5]:
            engine = Convex(cash=100000000,margin_rate=1,margin_limit=0,debug=False)
            engine.context.N=n
            engine.context.M = m
            engine.context.name=f"convexlong_N{n}_M{m}"
