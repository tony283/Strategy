...
df_sigma = pd.DataFrame(index=[f"Rangeuni"],columns=[f"Sigma{i}" for i in [5,20,40,63,126,252]])
...
for s in [5,20,40,63,126,252]:
    ...
    df_profit.loc[f"Rangeuni",f"Sigma{s}"]=profit
    df_sigma.loc[f"Rangeuni",f"Sigma{s}"]=sigma
    df_maxdrawdownrate.loc[f"Rangeuni",f"Sigma{s}"]=maxdrawdownrate
    df_pro_div_sigma.loc[f"Rangeuni",f"Sigma{s}"]=pro_div_sigma    
