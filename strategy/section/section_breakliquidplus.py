        ...
        if context.fired:
            if context.count<context.H:
                for future_type_dir, amount in self.position.hold.items():
                    ...
                    open = m_data[future_type]["open"].iloc[-1]
                    close=m_data[future_type]["close"].iloc[-1]
                    prev_close = m_data[future_type]["prev_close"].iloc[-1]
                    if(amount[0]//multi<=1):
                        continue
                    d = 1 if direction=="long" else -1
                    if (open-prev_close)* d <0:
                        self.sell_target_num(m_data[future_type]["open"].iloc[-1],amount[0]//(2*multi),multi,future_type,direction,
                                             is_close=False,close=close)
                return
            else:
                for future_type_dir, amount in self.position.hold.items():
                    info = future_type_dir.split("_")
...
if(__name__=="__main__"):
    ...
    for n in [i for i in range(1,21)]:
        for h in [0.1,0.15,0.2,0.25]:
            ...
            engine.context.L = n
            engine.context.name = f"newsecbreakliquidplus_Range{h:.2f}_Liquidity{n}"
            p.apply_async(engine.loop_process,args=("20120101","20240501","back/section/newsecbreakliquidplus/"))
            # engine.loop_process(start="20150101",end="20231231",saving_dir="back/section/newsecbreakliquidplus/")
    # print("-----start-----")
    # p.close()
    # p.join()
    # print("------end------")
